...
@Slf4j
@Component
public class NoTokenRequiredFilter extends AbstractGatewayFilterFactory<NoTokenRequiredFilter.Config> {
    ...
    public NoTokenRequiredFilter(TokenProvider tokenProvider, RedisTemplate redisTemplate) {
        super(Config.class);
...
//
//                        redisTemplate.opsForValue().set(newAccessToken, refreshToken, expiration, TimeUnit.MILLISECONDS);
//
...
//            request.mutate().header("userId", "NONE").build();
            return chain.filter(exchange);
        };
    }
